    ...
    <div class="container">
      ...
      <div v-else>
        ...
        <div v-else>
          ...
          <p>
            Go to
            <a href="https://sankeymatic.com/build/" target="_blank"
              rel="noopener noreferrer">sankeymatic.com/build/</a>
            and load the file.
          </p>
          <h2>Preview</h2>
...
export default {
  ...
  methods: {
    ...
    async selectMonth() {
      ...
      try {
        const { data: categoryData } = await this.api.categories.getCategories(
          this.budgetId
        );
        const { data: monthData } = await this.api.months.getBudgetMonth(
          this.budgetId,
          this.selectedMonth
        );
        ...
        this.sankeyFlows = this.generateSankeyFlows(
          categoryData.category_groups,
          monthData.month
        );
      } catch (err) {
        this.error =
    ...
    generateSankeyFlows(categoryGroups, month) {
      ...
      categoryGroups.forEach((group) => {
        if (group.hidden || group.deleted || !categoriesByGroupId[group.id])
          return;
